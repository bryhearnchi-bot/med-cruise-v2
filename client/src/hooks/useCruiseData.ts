...
export function transformCruiseData(data: CruiseData) {
  ...
  const importantInfo = {
    checkIn: {
      location: "Cruise Terminal A - World Trade Center Barcelona",
      address: "Moll de Barcelona, s/n, 08039 Barcelona, Spain",
      time: "2:00 PM - 5:00 PM",
      documents: ["Passport", "Cruise documents", "Proof of citizenship"]
    },
    departure: {
      sailAway: "7:00 PM",
      allAboard: "5:30 PM"
  ...
  const cruiseInfo = {
    ship: {
      name: data.cruise.shipName,
      line: data.cruise.cruiseLine,
...
export function useCruiseData(slug: string = getCruiseSlug()) {
  return useQuery<CruiseData>({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/cruises/${slug}/complete`);
      if (!response.ok) {
