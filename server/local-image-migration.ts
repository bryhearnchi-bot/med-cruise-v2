...
async function downloadAndSaveImage(cruise: CruiseImage): Promise<{ filename: string; localUrl: string }> {
  try {
    ...
    const filePath = path.join(process.cwd(), 'public', 'cruise-images', filename);
    console.log(`Saving ${filename} to ${filePath}...`);
    
    await fs.writeFile(filePath, buffer);
    
    const localUrl = `/cruise-images/${filename}`;
...
export async function migrateAllImages(): Promise<void> {
  ...
  for (const cruise of cruiseImages) {
    try {
      const { filename, localUrl } = await downloadAndSaveImage(cruise);
  ...
  console.log('Local image migration completed!');
}
